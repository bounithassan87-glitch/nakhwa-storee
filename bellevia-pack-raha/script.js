/* ==========================================================================
   ...
   Same engine as the sibling BelleVia pages: same order payload, same shared
   nk-track.js, same validation, same success/error contract. Only the product
   identity differs.
   ========================================================================== */
(function () {
  'use strict';
  ...
  var CFG = {
    ...
    productSlug: String(RAW.productSlug || 'bellevia-pack-raha').trim(),
    source: String(RAW.source || 'bellevia-pack-raha').trim(),
  ...
  /** "349 درهم" — an Arabic phrase, so it inherits the page's RTL. */
  function money(n) {
  ...
  if (CFG.delivery) {
    ...
    // The FAQ entry that asks about delivery only makes sense once there is a
    // delivery line to answer it with.
    $$('[data-delivery-q]').forEach(function (el) { el.hidden = false; });
  }
  ...
  (function prices() {
    ...
    // The struck-through anchor only appears when it is genuinely higher than
    // what is charged. A "was" price below the price is not a discount.
    //
    // The saving is SUBTRACTED, never typed: the badge cannot claim a discount
    // the two configured prices do not actually produce, and editing either
    // price in config.js moves it automatically.
    ...
    if (old !== null && UNIT !== null && old > UNIT) {
      ...
      var saved = Math.round((old - UNIT) * 100) / 100;
      $$('[data-save]').forEach(function (el) { el.textContent = 'وفّر ' + money(saved); el.hidden = false; });
    }
  ...
  var CONTENT = {
    content_name: 'PACK RAHA — BelleVia',
  ...
  (function orderForm() {
    ...
    // Every message the customer can see is in the neutral form of Darija — the
    // pack is «للجنسين» and the page is read by men and women alike.
    var RULES = {
      fullname: function (v) {
        if (!v) return 'عمّر السميّة ديالك.';
        if (v.length < 3) return 'السميّة قصيرة بزاف.';
        if (!/[؀-ۿa-zA-Z]/.test(v)) return 'كتب السميّة بالحروف.';
        return '';
      },
      phone: function (v) {
      ...
      city: function (v) {
        ...
        if (v.length < 2) return 'كتب سميّة المدينة كاملة.';
        return '';
      },
      address: function (v) {
    ...
    var SERVER_MESSAGES = {
      product_unavailable: 'المنتوج ماشي متوفر دابا. عيّط لينا وغادي نعاونوك.',
      ...
      invalid_quantity: 'هاد الكمية ماشي متوفرة. بدّلها وعاود.',
      rate_limited: 'بزاف ديال المحاولات فوقت قصير. تسنا شوية وعاود.',
      insufficient_stock: 'ما بقاش بزاف فالمخزون. نقّص الكمية ولا عيّط لينا.',
    };
    ...
    on(form, 'submit', function (e) {
      ...
      fetch(CFG.orderApiEndpoint, {
        ...
        .then(function (r) {
          ...
          showFormError(SERVER_MESSAGES[code] || 'ما قدرناش نسجلو الطلب دابا. عاود حاول ولا عيّط لينا.');
        })
        .catch(function () {
