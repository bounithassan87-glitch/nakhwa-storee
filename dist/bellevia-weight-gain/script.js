/* ==========================================================================
   Bellevia Weight Gain — behaviour
...
(function () {
  ...
  var CFG = {
    ...
    productSlug: String(RAW.productSlug || 'bellevia-weight-gain').trim(),
    source: String(RAW.source || 'bellevia-weight-gain').trim(),
  ...
  (function offer() {
    ...
    if (price) $$('[data-offer-mini]').forEach(function (el) { el.hidden = false; });
  ...
  /* Live total above the submit button. */
  (function orderTotal() {
    var box = $('#orderTotal');
    var val = $('#orderTotalVal');
    if (!box || !val || !PACKS.length) return;
    onPackChange(function (p) {
      val.textContent = money(p.total);
      box.hidden = false;
    });
  ...
  (function orderForm() {
    ...
    function setBusy(state) {
      ...
      submitBtn.innerHTML = state ? 'كنصيفطو الطلب…' : submitLabel;
    }
